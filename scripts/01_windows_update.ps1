...
# PSWindowsUpdate is a well-established module that wraps the Windows Update API.
# Source: PSGallery (the official PowerShell module repository).
if (-not (Get-Module -ListAvailable -Name PSWindowsUpdate)) {
    Write-Host "[*] PSWindowsUpdate module not found. Installing..."

    # Ensure NuGet provider is available (required by Install-Module)
    Write-Host "[*] Ensuring NuGet provider is available..."
    Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force -Scope AllUsers | Out-Null

    # Trust PSGallery so Install-Module does not prompt interactively
